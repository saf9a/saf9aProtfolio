...
export function CaseStudyVisual({ metrics, stack }: CaseStudyVisualProps) {
  return (
      ...
      <div className="rounded-md border border-background/10 bg-background/[0.06] p-4 dark:border-border dark:bg-background/45">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-lg font-semibold">Delivery cockpit</p>
          </div>
          <span className="rounded-full border border-background/15 px-3 py-1 text-xs text-background/75 dark:border-border dark:text-muted-foreground">
            Live
          </span>
        </div>
        <div className="mt-6 grid grid-cols-3 gap-2">
          {metrics.map((metric) => (
            <div key={metric} className="rounded-md border border-background/10 bg-background/[0.06] p-3 dark:border-border">
              <p className="text-xs leading-5 text-background/75 dark:text-muted-foreground">
                {metric}
              </p>
            </div>
          ))}
